...
1. **Внешний sidecar-процесс** `krisha_snapshots` (docker-compose на
   dev-сервере) обходит Krisha.kz по списку срезов из
   таблицы `estate.market.snapshot.config`: город + район (опц.) +
   тип объекта + комнаты.
2. Для каждого среза парсер считает `price / area` по ~100 объявлениям,
   фильтрует чужие комнатности (VIP/рекомендованные с другим количеством
   комнат), обрезает 5% выбросов, пишет медиану, P25, P75 **и сырой
   массив `samples_per_sqm`** в `estate.market.snapshot` (минимум 20
   объявлений на срез, иначе пропуск).
