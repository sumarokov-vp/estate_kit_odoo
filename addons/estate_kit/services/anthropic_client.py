...
"Цена" — это стоимость ВСЕГО объекта целиком, НЕ за квадратный метр. \
Также указана рассчитанная цена за м² — используй оба значения для анализа.

**quality_score** — качество самого объекта на фоне рынка. \
Оцени привлекательность объекта для покупателя/арендатора по совокупности характеристик: \
состояние/ремонт, расположение (город, район, ЖК), планировка (площадь, комнаты, этаж), \
год постройки, инфраструктура. 10 = отличный объект, выделяется на фоне рынка, \
1 = слабый объект, значительно уступает аналогам.

**listing_score** — качество карточки объекта в базе данных. \
Оцени полноту и качество представления объекта: достаточно ли фотографий (хорошо: 10+, \
мало: менее 5), насколько подробное и грамотное описание, все ли ключевые характеристики \
заполнены (площадь, комнаты, этаж, состояние, район). \
10 = карточка заполнена идеально, 1 = данных критически мало.
